...
void print_usage() {
	std::cerr << "\nUsage: psplit [OPTIONS] REGEX FILENAME FORMAT\n";
	std::cerr << "Options:\n";
...
void split_file(const std::string& regex_str, std::istream& input_stream, const std::string& format, bool verbose) {
    ...
    for (int index = 0; iter != end; ++iter) 
        create_and_write_file(format, index++, *iter, verbose);
}
...
int main(int argc, char* argv[]) {
    ...
    if (filename == "-") {
        split_file(regex_str, std::cin, format, verbose); // Read from standard input
    } else {
        ...
        split_file(regex_str, infile, format, verbose); // Read from the specified file
    }
